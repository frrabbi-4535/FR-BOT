...
async function startBot() {
  ...
  sock.ev.on("group-participants.update", async (u) => {
    ...
    for (const p of participants) {
      const n = p.split("@")[0]
      if (action === "add") {
        await sock.sendMessage(id, { text: `👋 Welcome @${n}`, mentions: [p] })
      }
      if (action === "remove") {
        await sock.sendMessage(id, { text: `👋 @${n} left the group`, mentions: [p] })
      }
      if (action === "promote") {
        await sock.sendMessage(id, { text: `🎉 Congrats @${n}, you are Admin now!`, mentions: [p] })
      }
    }
  })
